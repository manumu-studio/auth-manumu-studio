...
Implemented the Packet 02 schema foundation, first invite lifecycle service slice, transactional email outbox worker, and shared admission-control foundation for invite-gated registration. This adds the database state needed for account lifecycle control, invite redemption, transactional email outbox delivery with worker lease/fencing and retry fields, immutable audit events, explicit account linking, opaque registration sessions, and admin MFA factor posture/freshness; server-only invite issue/lookup/redeem/revoke helpers; an internal outbox worker with QStash-safe messages, claim-token fencing, encrypted invite delivery, and fragment-only invite links; and reusable Turnstile, CSRF, enumeration-parity, production-env, and rate-limit admission helpers. The user-facing invite flow is still future Packet 02 work; production remains protected by the existing registration kill switch until those runtime tasks ship.
...
| `src/features/auth/server/outbox/`, `src/app/api/internal/outbox-email/route.ts` | Created | QStash-ready transactional email worker, message/dedup helpers, claim/fencing state transitions, encrypted invite delivery, and worker route |
| `tests/gated-registration-outbox.test.ts` | Created | 12 outbox route, QStash, claim/fencing, retry, encryption, key-rotation, and fragment-link tests |
...
| `README.md` | Updated | Current capabilities, structure, outbox worker, and test count |
...
| `docs/TESTING.md` | Updated | 17 files / 194 tests and new gated-registration suites |
| `docs/cursor-task-reports/PACKET-02-gated-registration-reports/TASK-016-report.md`, `TASK-017-report.md`, `TASK-018-report.md`, `TASK-021-report.md` | Updated/created | Internal task evidence and handoffs |
...
- **Outbox schema contract repair:** filed and resolved `INCIDENT-P005` after TASK-018 preflight found missing lease/fencing and retry fields. TASK-016 now exposes `claimToken`, `leaseExpiresAt`, `nextAttemptAt`, `failedAt`, and `inviteCiphertext`; rerun Neon migration proof before production release because the initial migration changed after the prior proof.
- **Outbox worker scope:** TASK-018 implements delivery and QStash-safe enqueue helpers only; it emits `/invite#token=...` links but does not implement fragment exchange, registration-session cookies, GET validation, or redemption.
...
pnpm exec vitest run tests/gated-registration-outbox.test.ts # 12/12 passed
...
pnpm lint                            # passed
pnpm test                            # 194/194 passed across 17 files
